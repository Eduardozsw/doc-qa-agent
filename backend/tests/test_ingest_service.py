...
from core.exceptions import ForbiddenError, FileTooLargeError, UnsupportedFileTypeError, SlotLimitError
...
@pytest.mark.asyncio
async def test_ingest_rejects_when_no_slots():
    file = _make_upload()
    with patch("services.ingest.redis_db.get_namespaces", return_value=[f"f{i}.pdf" for i in range(5)]):
        from services.ingest import ingest_files
    ...
    with patch("services.ingest.redis_db.get_namespaces", return_value=[]):
        with patch("services.ingest.redis_db.get_cached_namespace", return_value="doc.pdf"):
            with patch("services.ingest.redis_db.add_namespace") as mock_add:
                with patch("services.ingest.load_document_from_bytes") as mock_load:
                    from services.ingest import ingest_files
...
async def test_ingest_new_file_processes_and_caches():
    ...
    with patch("services.ingest.redis_db.get_namespaces", return_value=[]):
        with patch("services.ingest.redis_db.get_cached_namespace", return_value=None):
            with patch("services.ingest.load_document_from_bytes", return_value="texto do pdf"):
                with patch("services.ingest.chunk_text", return_value=["chunk1", "chunk2"]):
...
async def test_ingest_raises_on_empty_text():
    ...
    with patch("services.ingest.redis_db.get_namespaces", return_value=[]):
        with patch("services.ingest.redis_db.get_cached_namespace", return_value=None):
            with patch("services.ingest.load_document_from_bytes", return_value="   "):
                from services.ingest import ingest_files
