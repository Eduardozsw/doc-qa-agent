...
async def test_query_with_owned_namespaces_succeeds():
    ...
    with patch("services.query.supabase_db.get_namespaces", return_value=["contrato.pdf"]):
...
async def test_query_with_unauthorized_namespace_raises():
    ...
    with patch("services.query.supabase_db.get_namespaces", return_value=["meu-doc.pdf"]):
...
async def test_query_with_empty_namespaces_skips_check():
    ...
    with patch("services.query.supabase_db.get_namespaces") as mock_supabase:
...
async def test_query_partial_unauthorized_namespaces_raises():
    ...
    with patch("services.query.supabase_db.get_namespaces", return_value=["meu.pdf"]):
        with pytest.raises(ForbiddenError):
            await handle_query(USER_ID, "free", req)
