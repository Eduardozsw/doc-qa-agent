...
async def summarize_document(user_id: str, plan: str, namespace: str) -> dict:
    ...
    cached = namespaces_db.get_summary(user_id, namespace)
    if cached:
        return {**cached, "cached": True}

    used = namespaces_db.count_summaries_this_month(user_id)
    ...
    namespaces_db.save_summary(user_id, namespace, summary)

    return {**summary, "cached": False}


async def list_summaries(user_id: str, plan: str) -> dict:
    summaries = namespaces_db.get_namespaces_with_summaries(user_id)
    used = namespaces_db.count_summaries_this_month(user_id)
    limit = get_limit(plan, "summaries")
    return {
        "summaries": summaries,
